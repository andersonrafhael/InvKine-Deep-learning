...
if __name__ == "__main__":
    ...
    for i, row in tqdm(test_data.iloc[:100,:].iterrows(), total=test_data.shape[0], desc="Running Api benchmark..."):
        ...
        api_time, api_thetas, time_inf = get_api_prediction(URL, data)
        api_bench_data.append(row.to_list() + list(api_thetas.values()) + [api_time, time_inf])
        
    api_bench_df = pd.DataFrame(api_bench_data, columns=["x", "y", "z", "theta0", "theta1", "theta2", "theta0_pred", "theta1_pred", "theta2_pred", "time_pred", "time_inf"])
